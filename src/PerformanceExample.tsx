...
const PerformanceExampleInner = ({
	...
	useEffect(() => {
		...
		if (!isSmall) {
			newRowsCols.push([10_000, 100], [50_000, newDefaultColCount], [100_000, newDefaultColCount])
		}
		...
		const loadFlags = async () => {
			try {
				const promiseArray = countries.map(async country => {
					...
					const response = await fetch(`https://flagcdn.com/w20/${countryCode}.png`, { signal: controller.signal })
